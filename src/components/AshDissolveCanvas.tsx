...
export const AshDissolveCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Cap DPR at 2: retina-crisp particles without quadruple fill cost on 3x screens.
    ...
    // On-demand render loop: spins only while particles are alive,
    // instead of burning a rAF tick on an empty stage forever.
    const render = () => {
    ...
    // Watch elements with data-kul="gecti" — checked on scroll (not every frame)
    ...
    // Automatic scroll-driven ash bursts are skipped for reduced-motion users;
    // explicit clicks (trigger-ash events) still work.
    if (!reduceMotion) {
      window.addEventListener('scroll', checkKulElements, { passive: true });
      checkKulElements();
    }
    ...
    return () => {
      ...
      window.removeEventListener('scroll', checkKulElements);
